            ...
            return try {
                ...
                if (response.isSuccessful) {
                    response.body()?.let { todos ->
                        saveTodos(todos) // Lagre hentede todos i databasen
                        return Result.Success(todoDao.getAllTodos())
                    } ?: Result.Error("Ingen todos funnet")
                } else {
                    Result.Error("Feil ved henting av todos: ${response.errorBody()?.string()}")
                }
            } catch (e: Exception) {
